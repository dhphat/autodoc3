...
const PROJECT_COLORS = [
  ...
  'bg-orange-100 text-orange-700', 'bg-pink-100 text-pink-70', 'bg-teal-100 text-teal-700',
...
const ContractsTab: React.FC<ContractsTabProps> = ({ profiles, departmentId }) => {
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      const [ct, at] = await Promise.all([
        downloadDefaultTemplate('contract', departmentId),
        downloadDefaultTemplate('acceptance', departmentId),
      ]);
  ...
  const handleTemplateUpload = async (file: File, type: 'contract' | 'acceptance') => {
    try {
      await uploadDefaultTemplate(file, type, departmentId);
