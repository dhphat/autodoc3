    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          // Gợi ý Google hiển thị tài khoản @fpt.edu.vn (hosted domain hint)
          queryParams: { hd: ALLOWED_DOMAIN },
        },
      });
      if (error) throw error;
      // Google redirect sẽ xảy ra ở đây — không cần finally setLoading
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md animate-slideUp">
        <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          {/* ── Lỗi domain từ Google OAuth (truyền từ App.tsx) ──── */}
          {authError && (
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 disabled:bg-gray-200 text-gray-700 disabled:text-gray-400 py-2.5 rounded-lg font-medium text-sm transition-all shadow-md mb-4 border border-white/20"
          >
            {googleLoading ? (
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/30 text-xs font-medium">HOẶC</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>
